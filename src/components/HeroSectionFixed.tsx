...
function HeroSection({ 
  ...
  useEffect(() => {
    ...
    const scheduleImageLoad = () => {
      // Only preload images on desktop, on mobile we'll load them when needed
      if (isMobile.current) {
  ...
  return (
    <section className="relative overflow-hidden min-h-[90vh] flex items-center justify-center text-white bg-navy-dark">
      {/* Hero background with overlay - optimized for mobile */}
      <div 
        className="absolute inset-0 bg-cover bg-center" 
        style={{ 
          opacity: opacity,
          // Simplified gradient for mobile to reduce painting cost
          backgroundImage: isMobile.current
            ? `linear-gradient(rgba(0, 8, 28, 0.8), rgba(0, 8, 28, 0.8)), url(${backgroundImage})`
            : `linear-gradient(rgba(0, 8, 28, 0.75), rgba(0, 8, 28, 0.75)), url(${backgroundImage})`,
          // Skip transition on mobile
          transition: isMobile.current ? 'none' : 'opacity 0.5s ease-in-out',
